...
if [ "$#" != 11 ]; then
    echo "usage: mean_separation.sh d_start d_end d_inc pe_start pe_end pe_inc run_start run_end run_inc in_dir out_dir"
    exit 1
fi
...
sep_exe="../bin/exe/mean_separation"
...
do
    in_path="${in_dir}/d_${d}/"
    if [ -d $in_path ]; then
	out_path="${out_dir}/d_${d}/separate/"
	if [ ! -d $out_path ]; then
	    mkdir -p $out_path
	fi
	pe=$(python -c "print '%.3f' % ($pe_start)")
	...
	do
		...
		sep_file="${out_path}/sep_${name}.dat"
		cmd[$jobid]="$sep_exe $N $lx $ly $tstart $tend $tinc $tshiftend $pos_file $pos_bulk_file $neigh_file $sep_file"
		jobid=$(bc <<< "$jobid + 1")
	    done
