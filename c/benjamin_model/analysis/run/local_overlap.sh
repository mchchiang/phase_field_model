...
if [ "$#" != 11 ]; then
    echo "usage: local_overlap.sh d_start d_end d_inc pe_start pe_end pe_inc run_start run_end run_inc in_dir out_dir"
    exit 1
fi
...
olap_exe="../bin/exe/local_overlap"
...
do
	...
	out_path="${out_dir}/local_overlap"
	if [ ! -d $out_path ]; then
	    mkdir -p $out_path
	fi
	pe=$(python -c "print '%.3f' % ($pe_start)")
	...
	do
		...
		if [ -f $pos_file ]; then
		    ...
		    out_file="${out_path}/local-overlap_${name}.dat"
		    cmd[$jobid]="$olap_exe $N $lx $ly $clx $cly $tstart $tend $tinc $ftinc $pos_file $field_path $field_file $out_file"
		    jobid=$(bc <<< "$jobid + 1")
		fi
	    done
